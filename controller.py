...
@app.route("/singleproduct/<int:i>")
def singleproduct(i):
    #Check if "user_id" key exists in session
    if "user_id" in session:
        #Query all products that belongs to specified category 
        product = Product.query.filter_by(id=i)
        ...
        return render_template("singleproduct.html", all_products = check, flag=False)
    else:
...
def product(i):
    ...
    if "user_id" in session:
        ...
        for i in check:
            print(i.id)
            qrcodeinfo.append(generate_qr_base64(i.id))
        #Render product.html template & pass the list of all products in specified category
        return render_template("product.html", all_products = check,qrcodeinfo=qrcodeinfo, flag=False)
    else:
        #If "user_id" key not exists in the session
        #redirect the user to sign-in page to authenticate before accessing the products
        return redirect("/sign-in")
